...
class BrainBizz_CorePlugin {
    ...
    static function activation_check() {
        if ( ! self::compatible_version() ) {
            ...
            wp_die( __( '"BrainBizz-core" Plugin requires with only "BrainBizz" theme!', 'brainbizz-core' ) );
        }
    }
    ...
    function disabled_notice() {
       echo '<strong>' . esc_html__( '"BrainBizz-core" Plugin requires with only "BrainBizz" theme!', 'brainbizz-core' ) . '</strong>';
    } 
...
new BrainBizz_CorePlugin();

register_activation_hook( __FILE__, array( 'BrainBizz_CorePlugin', 'activation_check' ) );
 ...
 */
function deactivate_brainbizz_core() {
    require_once plugin_dir_path( __FILE__ ) . 'includes/class-wgl-core-deactivator.php';
    BrainBizz_Core_Deactivator::deactivate();
}

register_activation_hook( __FILE__, 'activate_brainbizz_core' );
register_deactivation_hook( __FILE__, 'deactivate_brainbizz_core' );
...
function wgl_replace_url(&$data){
    ...
    foreach ($data as $key => &$value) {
        if(is_array($value)){
            ...
            $data[$key] = str_replace($theme_name.'.webgeniuslab.net', $output, $data[$key]);
        }
    }
    return $data;
}
 ...
 * @since    1.0.0
 */
function run_brainbizz_core() {

    $plugin = new BrainBizz_Core();
    $plugin->run();
