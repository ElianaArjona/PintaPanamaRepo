...
class BrainBizz_Core {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      BrainBizz_Core_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	public function __construct() {
		...
		$this->plugin_name = 'brainbizz-core';
	...
	public function wgl_ajax_init (){	
	    ...
	    list($query_args) = BrainBizz_Loop_Settings::buildQuery($atts);
	...
	public function wgl_mega_menu_load_ajax (){	
		...
		$out = '';
	    list($query_args) = BrainBizz_Loop_Settings::buildQuery($_POST);
	...
	public function wgl_cache_query($args = array()){
		...
		$custom_query = wp_cache_get( $cache_key, 'brainbizz_theme' );
		if ( false === $custom_query ) {
			$custom_query = new WP_Query( $args );
		    if ( ! is_wp_error( $custom_query ) && $custom_query->have_posts() ) {
		    	
		    	wp_cache_set( $cache_key, $custom_query, 'brainbizz_theme' );
		    }
		}
	  	return $custom_query;		
	}
	 ...
	 *
	 * - BrainBizz_Core_Loader. Orchestrates the hooks of the plugin.
	 * - BrainBizz_Core_i18n. Defines internationalization functionality.
	 * - BrainBizz_Core_Admin. Defines all hooks for the admin area.
	 * - BrainBizz_Core_Public. Defines all hooks for the public side of the site.
	 *
	...
	private function load_dependencies() {
		...
		$this->loader = new BrainBizz_Core_Loader();
	 ...
	 *
	 * Uses the BrainBizz_Core_i18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    1.0.0
	 * @access   private
	...
	private function set_locale() {
		...
		$plugin_i18n = new BrainBizz_Core_i18n();
	...
	private function define_admin_hooks() {
		...
		$plugin_admin = new BrainBizz_Core_Admin( $this->get_plugin_name(), $this->get_version() );
	...
	private function define_public_hooks() {
		...
		$plugin_public = new BrainBizz_Core_Public( $this->get_plugin_name(), $this->get_version() );
	 ...
	 *
	 * @since     1.0.0
	 * @return    BrainBizz_Core_Loader    Orchestrates the hooks of the plugin.
	 */
