...
<!-- Custom BTC FORM -->
		<?php
			$practices_terms = get_terms('team_category',array(
				'fields' => 'all',
				'name' => '',
				'slug' => ''
			) );
			$titles_terms = get_terms('titles_category', array(
				'fields' => 'all',
				'name' => '',
				'slug' => ''
			));
		?>
		<div class="wpcf7 btc_mrg">
				<div class="row">
					<div class="wgl_col-4">
						<span class="wpcf7-form-control-wrap">
							<select id="practices" class="wpcf7-form-control wpcf7-select">
							<option value="*">Todos los Estilos</option>
							<?php
								foreach($practices_terms as $key=>$tag):
								
							?>
								<option value="<?php echo normalize($tag->name); ?>"><?php echo $tag->name; ?></option>
							<?php
								endforeach;
							?>
							</select>
					...
					<div class="wgl_col-4">
						<span class="wpcf7-form-control-wrap">
							<select id="titles" class="wpcf7-form-control wpcf7-select">
								<option value="*">Cualquier Forma de Pago</option>
								<?php
								foreach($titles_terms as $key=>$tag):
								?>
									<option value="<?php echo normalize($tag->name);?>"><?php echo $tag->name; ?></option>
								<?php
									endforeach;
					...
					<div class="wgl_col-4">
						<span class="wpcf7-form-control-wrap">
							<input type="text" class="wpcf7-form-control wpcf7-text btc_search" placeholder="Nombre del Autor"/>
						</span>
					</div>
				</div>
		</div>
		...
		<div <?php echo esc_attr($team_id_attr); ?> class="wgl_module_team <?php echo esc_attr($team_classes); ?>">
		    <div class="team-items_wrap clearfix grid" <?php echo BrainBizz_Theme_Helper::render_html($style_gap);?> >
		        <?php
