...
/**
* BrainBizz Loop Settings
*
*
* @class        BrainBizz_Loop_Settings
* @version      1.0
* @category Class
* @author       WebGeniusLab
*/
if (!class_exists('BrainBizz_Loop_Settings')) {
    class BrainBizz_Loop_Settings{
        ...
        public static function buildQuery($query){
            $query_builder = new BrainBizz_Query_Builder( $query );
        ...
        public static function init($base_name, $array = array()){
            ...
            add_filter( 'vc_autocomplete_'.$base_name.'_by_posts_callback', 'BrainBizz_Loop_Settings::by_posts_suggester', 10, 1 );

            add_filter( 'vc_autocomplete_'.$base_name.'_by_posts_render', 'BrainBizz_Loop_Settings::by_posts_suggester_render', 10, 1 );

            //Add Autocomplete categories
            add_filter( 'vc_autocomplete_'.$base_name.'_categories_callback', 'BrainBizz_Loop_Settings::categories_suggester', 10, 1 );

            add_filter( 'vc_autocomplete_'.$base_name.'_categories_render', 'BrainBizz_Loop_Settings::categories_suggester_render', 10, 1 );

            //Add Autocomplete tags
           add_filter( 'vc_autocomplete_'.$base_name.'_tags_callback', 'BrainBizz_Loop_Settings::tags_suggester', 10, 1 );

            add_filter( 'vc_autocomplete_'.$base_name.'_tags_render', 'BrainBizz_Loop_Settings::tags_suggester_render', 10, 1 );            

            //Add Autocomplete taxonomies
           add_filter( 'vc_autocomplete_'.$base_name.'_taxonomies_callback', 'BrainBizz_Loop_Settings::taxonomies_suggester', 10, 1 );

            add_filter( 'vc_autocomplete_'.$base_name.'_taxonomies_render', 'BrainBizz_Loop_Settings::taxonomies_suggester_render', 10, 1 );          

            //Add Autocomplete users
           add_filter( 'vc_autocomplete_'.$base_name.'_author_callback', 'BrainBizz_Loop_Settings::author_suggester', 10, 1 );

            add_filter( 'vc_autocomplete_'.$base_name.'_author_render', 'BrainBizz_Loop_Settings::author_suggester_render', 10, 1 );

            vc_add_param($base_name,array(
                    'type'        => 'textfield',
                    'edit_field_class' => 'vc_col-sm-4',
                    'heading'     => esc_html__('Post count', 'brainbizz'),
                    'param_name'  => 'number_of_posts',
                    'description' => '',
                    'std'         => 12,
                    'group' => esc_html__( 'Query', 'brainbizz' ),
                )
            );
            vc_add_param($base_name,array(
                    'type'        => 'dropdown',
                    'edit_field_class' => 'vc_col-sm-4 no-top-padding',
                    'heading'     => esc_html__('Order By', 'brainbizz'),
                    'param_name'  => 'order_by',
                    'value'       => array(
                        esc_html__('Date', 'brainbizz')  => 'date',
                        esc_html__('Title', 'brainbizz') => 'title',
                        esc_html__('Author', 'brainbizz')  => 'author',
                        esc_html__('Modified', 'brainbizz')  => 'modified',
                        esc_html__('Random', 'brainbizz')  => 'rand',
                        esc_html__('Comment count', 'brainbizz')  => 'comment_count',
                        esc_html__('Menu order', 'brainbizz')  => 'menu_order',
                    ),
                    'save_always' => true,
                    'description' => '',
                    'group' => esc_html__( 'Query', 'brainbizz' ),
            ...
            vc_add_param($base_name,array(
                    'type'        => 'dropdown',
                    'edit_field_class' => 'vc_col-sm-4 no-top-padding',
                    'heading'     => esc_html__('Sort Order', 'brainbizz'),
                    'param_name'  => 'order',
                    'value'       => array(
                        esc_html__('ASC', 'brainbizz')  => 'ASC',
                        esc_html__('DESC', 'brainbizz') => 'DESC'
                    ),
                    'save_always' => true,
                    'description' => '',
                    'group' => esc_html__( 'Query', 'brainbizz' ),
            ...
            if(!isset($array['hide_cats'])){
                vc_add_param($base_name,array(
                        ...
                        'heading'     => esc_html__('Category Slug', 'brainbizz'),
                        'param_name'  => 'categories',
                        'description' => esc_html__('Leave empty for all', 'brainbizz'),
                        'group' => esc_html__( 'Query', 'brainbizz' ),
                        'settings' => array(
                            'multiple' => true,
                            'min_length' => 2,
                ...
                vc_add_param($base_name,array(
                        'type' => 'wgl_checkbox',
                        'heading' => esc_html__('Exclude', 'brainbizz' ),
            ...
            if(!isset($array['hide_tags'])){
                vc_add_param($base_name,array(
                        'type'        => 'autocomplete',
                        'edit_field_class' => 'vc_col-sm-10',
                        'heading'     => esc_html__('Tags Slug', 'brainbizz'),
                        'param_name'  => 'tags',
                        'description' => esc_html__('Leave empty for all', 'brainbizz'),
                        'group' => esc_html__( 'Query', 'brainbizz' ),
                        'settings' => array(
            ...
            vc_add_param($base_name,array(        
                    'type'        => 'autocomplete',
                    'edit_field_class' => 'vc_col-sm-10',
                    'heading'     => esc_html__('Taxonomies', 'brainbizz'),
                    'param_name'  => 'taxonomies',
                    'description' => esc_html__('Filter output by custom taxonomies categories, enter category names here.', 'brainbizz'),
                    'group' => esc_html__( 'Query', 'brainbizz' ),
                    'settings' => array(
                        'multiple' => true,
                        'min_length' => 2,
            ...
            vc_add_param($base_name,array(           
                    'type'        => 'autocomplete',
                    'class'       => '',
                    'heading'     => esc_html__('Individual Posts/Pages/Custom Post Types', 'brainbizz'),
                    'param_name'  => 'by_posts',
                    'edit_field_class' => 'vc_col-sm-10',
                    'description' => esc_html__('Individual Posts/Pages/Custom Post Types', 'brainbizz'),
                    'settings' => array(
                        'multiple' => true,
                        'min_length' => 2,
                        'display_inline' => false,
            ...
            vc_add_param($base_name,array(          
                    'type'        => 'autocomplete',
                    'class'       => '',
                    'heading'     => esc_html__('Author', 'brainbizz'),
                    'param_name'  => 'author',
                    'description' => esc_html__('Filter by author name.', 'brainbizz'),
                    'group' => esc_html__( 'Query', 'brainbizz' ),
                    'edit_field_class' => 'vc_col-sm-10',
                    'settings' => array(
                        'multiple' => true,
        ...
        public static function categories_suggester($query){
            ...
            foreach ( $categories as $cat ) {
                ...
                $content[] = array( 'value' => (string) $cat->slug, 'label' => $cat->cat_name.(!empty($parent) ? esc_html__(' (Parent categories: (', 'brainbizz') .$parent.'))' : ""));
            }
            return $content;            
        }
        ...
        public static function taxonomies_suggester($query){
            ...
            foreach ( $tags as $tag ) {
                ...
                $content[] = array(
                    ...
                    'label' => $tag->name . ' (' . $tag->taxonomy . ')'.(!empty($parent) ? esc_html__(' (Parent categories: (', 'brainbizz') .$parent.'))' : "")
                );
            }
            return $content;
        }
            ...
            if ( ! empty( $query ) ) {
                ...
                foreach ( $list as $obj ) {  
                    ...
                    $content[] = array( 
                        ...
                        'label' => $obj->name . ' (' . $obj->taxonomy . ')'.(!empty($parent) ? esc_html__(' (Parent categories: (', 'brainbizz') .$parent.'))' : "")
                    );
                }
                return isset($content[0]) ? $content[0] : false;  
            }
    ...
    new BrainBizz_Loop_Settings();
}

/**
* BrainBizz Query Builder
*
*
* @class        BrainBizz_Query_Builder
* @version      1.0
* @category Class
* @author       WebGeniusLab
*/
if(!class_exists('BrainBizz_Query_Builder')){
    class BrainBizz_Query_Builder{
        /**
         * @since 1.0
         * @var array
        ...
        private function get_tax( $value ){
            ...
            $taxonomies = get_terms( BrainBizz_Loop_Settings::getTaxonomies() );
            ...
            $terms = get_terms( BrainBizz_Loop_Settings::getTaxonomies(),
