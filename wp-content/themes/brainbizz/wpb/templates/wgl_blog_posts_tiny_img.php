...
$defaults = array(
    ...
    'name_load_more' => esc_html__('Load More', 'brainbizz'),
);
...
if( (bool)$custom_fonts_blog_content || (bool)$custom_fonts_blog_headings ) {
    ...
    extract( BrainBizz_GoogleFontsRender::getAttributes( $atts, $this, array('google_fonts_blog', 'google_fonts_blog_headings')));
...
$query = BrainBizz_Theme_Cache::cache_query($query_args);
    // Render Items blog
$wgl_def_atts = array(
    'query' => $query,
    ...
    <section class="wgl_cpt_section">
        <div <?php if ((bool)$blog_id) echo 'id="'.esc_attr($blog_id).'"' ?> class="blog-posts <?php echo esc_attr($css_class); ?>">
            ...
            if(!empty($blog_title) || !empty($blog_subtitle)){
                ...
                if(!empty($blog_title)) echo '<h3 class="brainbizz_module_title blog_title">'.wp_kses( $blog_title, $allowed_html ).'</h3>';
            ...
            echo '<div class="container-grid row '. esc_attr($row_class) .'">';
                echo BrainBizz_Theme_Helper::render_html($blog_items);
            echo '</div>';
            ?>
        </div>
<?php
...
if ( $blog_navigation == 'pagination' ) {
    echo BrainBizz_Theme_Helper::pagination('10', $query, $blog_navigation_align);
}
...
if ( $blog_navigation == 'load_more' ) {
    ...
    echo BrainBizz_Theme_Helper::load_more('10', $wgl_blog_atts, $blog_navigation_align, $name_load_more, $class);
}
    echo '</section>';
endif;
?>
