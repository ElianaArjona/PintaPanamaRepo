<?php
	...
	$counter_icon = $counter_title = $counter_value = $counter_value_hide = '';
	...
	if ((bool)$custom_title_color || (bool)$custom_count_value_color || (bool)$custom_icon_color || (bool)$custom_border_color || $bg_color_type != 'def') {
		$counter_id = uniqid( "brainbizz_counter_" );
	...
	// Render Google Fonts
	extract( BrainBizz_GoogleFontsRender::getAttributes( $atts, $this, array('google_fonts_title', 'google_fonts_count_value') ) );
	...
	$counter_value .= !empty($count_value) ? '<div class="counter_value_wrap" '.$count_value_styles.'>'.(!empty($count_prefix) ? '<span>'.esc_html($count_prefix).'</span>' : '').'<span class="counter_value">'.esc_html($count_value).'</span>'.(!empty($count_suffix) ? '<span class="counter_value_suffix">'.esc_html($count_suffix).'</span>' : '').'</div>' : '';

	$counter_value_hide .= !empty($count_value) ? '<div class="counter_value_hide" '.$count_value_styles.'>'.(!empty($count_prefix) ? '<span>'.esc_html($count_prefix).'</span>' : '').'<span class="counter_value">'.esc_html($count_value).'</span>'.(!empty($count_suffix) ? '<span class="counter_value_suffix">'.esc_html($count_suffix).'</span>' : '').'</div>' : '';
		...
		case 'top_right':
			...
			$counter_inner .= '<div class="counter_content_wrapper">';
				$counter_inner .= '<div class="counter_value_content">';
					$counter_inner .= $counter_value;
					$counter_inner .= $counter_value_hide;
				$counter_inner .= '</div>';
			$counter_inner .= $counter_title;
			$counter_inner .= '</div>';
			break;
	...
	// Render html
	$output .= '<div '.esc_attr($counter_id_attr).' class="brainbizz_module_counter'.esc_attr($counter_wrap_classes).'">';
