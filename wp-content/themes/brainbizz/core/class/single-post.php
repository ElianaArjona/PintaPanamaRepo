...
if (!class_exists('BrainBizz_Single_Post')) {

	class BrainBizz_SinglePost{
		/**
	     * @var SinglePost
	     */
	    ...
	    /**
	    * @return SinglePost category
		* @access public
		*/
		public $cat_render_image = false;
	    ...
	    public function set_data ($link_feature = false) {
	   	...
	   	public function set_data_image ($link_feature = false, $image_size = 'full', $image_with_cat = false, $aq_image = false) {
	    	$output = '';
	    ...
	    public function render_featured ( $link_feature = false, $image_size = 'full', $aq_image = false ) {
	    	$output = '';
			if (class_exists( 'RWMB_Loader' )) {
		    	switch($this->post_format) {
		            case 'gallery' :
	 		...
	 		if (!empty($media)){
	 			echo '<div class="blog-post_media"><div class="blog-post_media_part'.esc_attr($class_media_part).'">' . $media; 
	 			echo '</div>'; 
	 			echo '</div>'; 	 			
	 		} 	 		
	    }	   
	    ...
	    public function videobox(){
	    	if($this->post_format === 'video'){
	    		...
	    		if($video_style != 'bg_video'){
					...
					$uniqrel = uniqid();
		    		echo '<div class="brainbizz_module_videobox button_align-center">';
	    ...
	    public function render_bg ( $link_feature = false, $image_size = 'full',$aq_image = false, $data_animation = null ) {
	    	...
	    	if($this->post_format === 'gallery' && !is_single()){
	    		...
	    		if($this->post_format === 'video' && $video_style == 'bg_video'){
	    			echo BrainBizz_Theme_Helper::render_html($media);
	    		}else{
	    	...
	    	if(is_single() && $video_style != 'bg_video'){
	    		...
	    		if($video_style == 'bg_video'){
			    	...
			    	if(has_post_thumbnail()) {
			    		...
			    		$uniqrel = uniqid();
			    		$output .= '<div class="brainbizz_module_videobox button_align-center with_image">';
			    		if(isset($video_link[0])){
			    			$output .= '<a data-rel="youtube-'.esc_attr($uniqrel).'" href="'.esc_url($video_link[0]) .'" class="videobox_wrapper_link videobox">';
			    		}
	    ...
	    public function featured_gallery ($link_feature, $image_size) {
	    	...
	    	$arr = $this->image_size_render($image_size);  
			extract($arr);
			$class = 'blog-post_media-slider_slick brainbizz_carousel_slick fade_slick';
			$class_module = ' brainbizz_module_carousel';
			ob_start();
			?>
			...
			<?php
           	...
           	?>
           		<div class="blog-post_quote-author">
           			<?php
           			echo BrainBizz_Theme_Helper::render_html($autor_avatar);
	    ...
	    public function render_post_meta_date () {?>
	    ...
	    public function render_post_meta ($args = false) {
	    	...
	    	if($this->show_author_meta || $this->show_comments_meta){
		    	?>
		    	<div class="meta-wrapper">
					<?php if($this->show_author_meta) : ?>
						<span class="author_post"><?php echo esc_html__('by', 'brainbizz'); ?> <a href="<?php echo esc_url(get_author_posts_url(get_the_author_meta('ID'))); ?>"><?php echo esc_html(get_the_author_meta('display_name')); ?></a></span>
					<?php endif; ?>

					<?php if($this->show_comments_meta) :
						$comments_num = '' . get_comments_number($this->post_id) . '';
						$comments_text = '' . $comments_num == 1 ? esc_html__('Comment', 'brainbizz') : esc_html__('Comments', 'brainbizz')  . '';
					?>
	    ...
	    public function render_post_cats () {
			...
			if ($categories) {
				echo '<div class="current-menu-item"></div>';
	    ...
	    public function render_excerpt ($symbol_count = false, $shortcode = false) {
			...
			if ((bool)BrainBizz_Theme_Helper::get_option('blog_post_listing_content') || $shortcode) {
				$post_content = preg_replace( '~\[[^\]]+\]~', '', $post_content);
				$post_content_stripe_tags = strip_tags($post_content);
				$output = BrainBizz_Theme_Helper::modifier_character($post_content_stripe_tags, $symbol_count, "...");
			} else {
				$output = $post_content;
			}
	    ...
	    public function get_post_views($postID){
		    ...
		    if($count==''){
		    	...
		    	echo '<span class="counts_text">'. esc_html__(' View', 'brainbizz'). '</span>';
		    }else{
				echo '<span class="counts">'. esc_html($count). '</span>';
		    	echo '<span class="counts_text">'. esc_html__(' Views', 'brainbizz'). '</span>';
		    }
		}
	    ...
	    public function render_author_info () {
			...
			$social_html = !empty($social_html) ? '<div class="author-info_social-wrapper">'.'<span class="title_soc_share">'.esc_html__('In Socials:', 'brainbizz').'</span>'. $social_html.'</div>' : '';
