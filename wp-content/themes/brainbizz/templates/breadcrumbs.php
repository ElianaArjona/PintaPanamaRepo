...
*
*
* @class 		BrainBizz_breadcrumbs
* @version		1.0
* @category	Class
* @author 		WebGeniusLab
*/
if (!class_exists('BrainBizz_breadcrumbs')) {
    class BrainBizz_breadcrumbs {
		...
		private function text_options(){
			$this->text_home     = esc_html__('Home', 'brainbizz'); // text for the 'Home' link
			$this->text_category = esc_html__('Archive by Category "%s"', 'brainbizz'); // text for a category page
			$this->text_search   = esc_html__('Search Results for "%s" Query', 'brainbizz'); // text for a search results page
			$this->text_tag      = esc_html__('Posts Tagged "%s"', 'brainbizz'); // text for a tag page
			$this->text_author   = esc_html__('Articles Posted by %s', 'brainbizz'); // text for an author page
			$this->text_404      = esc_html__('Error 404', 'brainbizz'); // text for the 404 page
			$this->text_page     = esc_html__('Page %s', 'brainbizz'); // text 'Page N'
			$this->text_cpage    = esc_html__('Comment Page %s', 'brainbizz'); // text 'Comment Page N'		
		}
    	public function __construct () {				
			...
			$home_link      = $link_before . '<a href="' . esc_url($home_url) . '"' . $link_attr . ' class="home">' . $link_in_before . $this->text_home . $link_in_after . '</a>' . $link_after;
				...
				// custom post type
	   			...
	   			elseif (function_exists('is_product') && is_product()) {
	   				if(function_exists('brainbizz_woocommerce_breadcrumb')){
	   					...
	   					$args['delimiter'] = $sep;
	   					if ($show_home_link) $out .= $sep;
	   					$out .= brainbizz_woocommerce_breadcrumb($args);
	   				}
	   			}
				$out .= $wrap_after;

				echo BrainBizz_Theme_Helper::render_html($out);
			}
    	}
    }
    new BrainBizz_breadcrumbs();
}