...
/**
* BrainBizz Theme Helper
*
*
* @class        BrainBizz_Theme_Helper
* @version      1.0
* @category Class
* @author       WebGeniusLab
*/
...
if (!class_exists('BrainBizz_Theme_Helper')) {
    class BrainBizz_Theme_Helper{
        ...
        public static function get_option($name, $preset = null, $def_preset = null) {
            if (  class_exists( 'Redux' ) && class_exists( 'BrainBizz_Core_Public' ) ) {
                $preset = $preset == 'default' ? null : $preset;

                if (!$preset) {

                    // Customizer
                    if (!empty($GLOBALS['brainbizz_set']) && $GLOBALS['brainbizz_set'] != NULL) {
                        $theme_options = $GLOBALS['brainbizz_set'];
                    } else {
                        $theme_options = get_option( 'brainbizz_set' );
                    }

                } else {
                    $theme_options = get_option( 'brainbizz_set_preset' );
                }
                
                if (empty($theme_options)) {
                    $theme_options = get_option( 'brainbizz_default_options' );
                }
                ...
                $default_option = get_option( 'brainbizz_default_options' );
        ...
        public static function bg_render($name,$check_key = false,$check_value = false){
            $image = BrainBizz_Theme_Helper::get_option($name."_bg_image");
            ...
            if (self::get_option('preloader') == '1' || self::get_option('preloader') == true) {
                ...
                echo '<div id="preloader-wrapper" '.esc_attr($bg_styles).'>
                        <div class="preloader-container">
                            <img class="preloader-sz" src="/wp-content/uploads/2019/11/logoTopicosenPng23.png" alt="Loader" />
                          </div>
                        </div>
                    </div>';
            }
        ...
        public static function load_more($range = 5, $query = false, $alignment = 'left', $name_load_more = '', $class = ''){
            $out = '';
            $name_load_more = !empty($name_load_more) ? $name_load_more : esc_html__("Load More", "brainbizz");
        ...
        public static function header_preset_name(){
            ...
            if( !self::in_array_r($name_preset, get_option( 'brainbizz_set_preset' ))){                   
                $name_preset = 'default';
            }
        ...
        public static function render_sidebars($args = 'page'){
            ...
            $brainbizz_core = class_exists('BrainBizz_Core');  
        ...
        public static function posted_meta_on(){
            global $post;
            $text_string = '<span><time class="entry-date published" datetime="%1$s">%2$s</time></span><span>' . esc_html__('Published in', 'brainbizz') . ' <a href="%3$s" rel="gallery">%4$s</a></span>';
            ...
            if ( $metadata ) {
                printf( '<span class="full-size-link"><span class="screen-reader-text">%1$s </span><a href="%2$s" title="%2$s">%1$s %3$s &times; %4$s</a></span>',
                    esc_html_x( 'Full size', 'Used before full size attachment link.', 'brainbizz' ),
            ...
            edit_post_link(
                sprintf(
                    ...
                    wp_kses( __( 'Edit<span class="screen-reader-text"> "%s"</span>', 'brainbizz' ), $allowed_html ) ,
                        get_the_title()
        ...
        public function vc_iconpicker_type_flaticon( $icons ) {
            $flaticon_icons = array(
                ...
                array('flaticon-mouse-1' => 'mouse-1'),
                array('flaticon-mouse-2' => 'mouse-2'),
                ...
                array('flaticon-click' => 'click'),
                array('flaticon-check' => 'check'),
                array('flaticon-information-circular-button-symbol' => 'information-circular-button-symbol'),
                array('flaticon-close-cross-circular-interface-button' => 'close-cross-circular-interface-button'),
                array('flaticon-acceptance-circular-button' => 'acceptance-circular-button'),
                array('flaticon-right-arrow-circular-button' => 'right-arrow-circular-button'),
                array('flaticon-back-button' => 'back-button'),
                ...
                array('flaticon-check-2' => 'check-2'),
                ...
                array('flaticon-trash' => 'trash'),
                array('flaticon-star' => 'star'),
                array('flaticon-photo-camera' => 'photo-camera'),
                array('flaticon-ticket-1' => 'ticket-1'),
                array('flaticon-agreement' => 'agreement'),
                array('flaticon-news' => 'news'),
                array('flaticon-handshake' => 'handshake'),
                array('flaticon-rocket' => 'rocket'),
                array('flaticon-world' => 'world'),
                array('flaticon-arrow' => 'arrow'),
                array('flaticon-arrow-down-sign-to-navigate' => 'arrow-down-sign-to-navigate'),
                array('flaticon-arrowhead-thin-outline-to-the-left' => 'arrowhead-thin-outline-to-the-left'),
                array('flaticon-navigate-up-arrow' => 'navigate-up-arrow'),
                array('flaticon-quotation-mark' => 'quotation-mark'),
                array('flaticon-bag-1' => 'bag-1'),
                array('flaticon-sale' => 'sale'),
                ...
                array('flaticon-magnifying-glass-1' => 'magnifying-glass-1'),
                array('flaticon-star-1' => 'star-1'),
                array('flaticon-discount' => 'discount'),
                ...
                array('flaticon-chart' => 'chart'),
                array('flaticon-coin' => 'coin'),
                array('flaticon-chart-1' => 'chart-1'),
                ...
                array('flaticon-yield' => 'yield'),
                array('flaticon-speed' => 'speed'),
                array('flaticon-accept' => 'accept'),
                array('flaticon-quote-1' => 'quote-1'),
