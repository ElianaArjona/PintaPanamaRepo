<?php
	...
	if ((bool)$custom_quote_color || (bool)$custom_name_color || (bool)$custom_status_color || (bool)$custom_quote_icon_color) {
		$testimonials_id = uniqid( "brainbizz_testimonials_" );
	...
	// Render Google Fonts
	extract( BrainBizz_GoogleFontsRender::getAttributes( $atts, $this, array('google_fonts_name', 'google_fonts_status', 'google_fonts_quote') ) );
	...
	// Image styles
	$image_width_crop = ($custom_img_width != '') ? $custom_img_width*2 : '240';
	$image_width = ($custom_img_width != '') ? 'width:'.(int)$custom_img_width.'px; ' : 'width:120px; ';
	...
	$output .= '<div '.esc_attr($testimonials_attr).' class="brainbizz_module_testimonials'.esc_attr($testimonials_wrap_classes).'">';
		if ((bool)$use_carousel) {
